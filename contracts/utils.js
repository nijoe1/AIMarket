...
const _subscriptionIdFuji = 4075;

const codeStringT = `
  const apiResponse = await Functions.makeHttpRequest({
    url: 'https://aimarket-avalanche.vercel.app/api/functions/getTopAgents'
  });
...
const codeStringR = `
  const apiResponse = await Functions.makeHttpRequest({
    url: 'https://aimarket-avalanche.vercel.app/api/functions/getTopAgentsByRatings'
  });
...
const codeStringU = `
  const apiResponse = await Functions.makeHttpRequest({
    url: 'https://aimarket-avalanche.vercel.app/api/functions/getTopUsers'
  });
...
const deployment_config_Fuji = [_oracleFuji, _unlockContractFuji, _donIDFuji, _subscriptionIdFuji];
